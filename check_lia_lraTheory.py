...
ineq_const = make_ineq("<=", dict(zip(["0"],[Fraction("0")])), dict(zip(["1"],[Fraction("1")]))) # 0 <= 1, used in negIneq

def negIneq(ineq):
    return make_ineq("<=",snd(ineq), mult(add(thd(ineq),thd(ineq_const)), Fraction("-1"))) # specific for LIA: not(0 <= 1 + x) ---> 0 <= (1-1) - x
